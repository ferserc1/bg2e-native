# Step 7: Modify RTReflections to Use Material Data Binding
...
void render(
    ...
    const std::vector<vulkan::rt::RTObjectInstance>& objectInstances  // NEW
);
```
...
1. **Set numbering**: Set 0 is the existing TLAS + output + G-buffer set. Set 1 is the new material data set with 4 array bindings. The pipeline layout must have them in this order.
...
3. **Empty object list**: If `objectInstances` is empty, we skip binding the material set. The shader should handle this case (e.g., use a default material).
...
6. **Array bindings**: The material data binding uses 4 array bindings (binding 0: SSBO count=1, binding 1: SSBO count=256, binding 2: SSBO count=256, binding 3: CIS count=256). The shader uses `GL_EXT_nonuniform_qualifier` to index into these arrays.
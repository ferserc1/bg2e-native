# Step 6: Create rt_material_data.glsl Shader Include
...
layout(set = 1, binding = 1) readonly buffer VertexBuffer0 {
    RTVertex vertices0[];
};

layout(set = 1, binding = 2) readonly buffer IndexBuffer0 {
    uint indices0[];
};

layout(set = 1, binding = 3) uniform sampler2D albedoTex0;

// ... etc for each material slot
```

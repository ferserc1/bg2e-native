...
Helper scripts live in `scripts/`. They are not part of the CMake build; run them from the repo root. See `scripts/PACKAGING.md` for full packaging details.

### Packaging (engine + apps distribution)

Builds a clean, self-contained package named `bg2engine_<12-char-HEAD-hash>` containing `libbg2e.so`/`.dll`, shaders and assets — no external system libraries. Requires a **clean git checkout** (no pending changes, submodules matching), a valid `VULKAN_SDK`, and a destination **outside the repo**. Builds in a fresh temp tree with `-DBG2E_PACKAGE_APPS=ON`, installs the Distribution component, strips all `.so` except `libbg2e.so` (Linux), writes `build-info.json`, and emits both an unpacked folder and an archive (`.tar.gz` on Linux/macOS, `.zip` on Windows).

| Script | Usage |
|--------|-------|
| `scripts/package_linux.sh` | `./scripts/package_linux.sh [--allow-dirty] [--overwrite] Debug\|Release DESTINATION [VULKAN_SDK]` |
| `scripts/package_macos.sh` | same CLI as Linux (macOS) |
| `scripts/package_windows.ps1` | `.\scripts\package_windows.ps1 -Configuration Debug\|Release -Destination <path> [-VulkanSdk <path>]` |

### Standalone project creation

Generates a standalone CMake project for building bg2e apps **outside** the repo, using pre-compiled libraries.

| Script | Usage |
|--------|-------|
| `scripts/create_standalone_project.sh` | `./scripts/create_standalone_project.sh <target_path> [project_name]` (default: `bg2e_app`) |
| `scripts/create_standalone_project.ps1` | `.\scripts\create_standalone_project.ps1 <target_path> [project_name]` |

### Library updating (standalone projects)

Rebuilds the bg2e library and refreshes headers + current-platform lib inside a project created by `create_standalone_project.*`. Uses a dedicated `build-update-libraries/` dir so it does not touch the CLion build tree or package temp trees.

| Script | Usage |
|--------|-------|
| `scripts/update_libraries.sh` | `./scripts/update_libraries.sh <standalone_project_path> Debug\|Release` (exactly 2 args) |
| `scripts/update_libraries.ps1` | `.\scripts\update_libraries.ps1 <standalone_project_path> -Configuration Debug\|Release [-VulkanSdk <path>]` |

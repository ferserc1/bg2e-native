...
void main() {
    ...
    float inputColor = texture(g_InputImage, uv).r;
    vec3 centerNormal = texture(g_Normal, uv).xyz * 2.0 - 1.0;
    float centerDepth = texture(g_Depth, uv).r;

    if (centerDepth >= 1.0) {
        imageStore(outImage, pixelCoord, vec4(1.0, 0.0, 0.0, 1.0));
        return;
    }
    ...
    for (int y = -radius; y <= radius; ++y) {
        for (int x = -radius; x <= radius; ++x) {
            ...
            float neighborInput = texture(g_InputImage, neighborUV).r;
            ...
            float depthWeight = depthDiff < pc.depthThreshold ? 1.0 : exp(-(depthDiff * depthDiff) / (2.0 * pc.depthSigma * pc.depthSigma));

            float normalDot = clamp(dot(centerNormal, neighborNormal), 0.0, 1.0);
            float normalWeight = normalDot > pc.normalThreshold ? 1.0 : exp(-(1.0 - normalDot) / pc.normalSigma);
    ...
    imageStore(outImage, pixelCoord, vec4(result, 0.0, 0.0, 1.0));
}